    ...
    public function addToTransfer($data)
{
    ...
    if ($rowsToSelect > 0) {     
        ...
        return ['product_id' => $data['product_id'], 'ean' => $data['ean'],'updated_rows' => $updatedRowCount, 'updated_rows_ids' => $updatedRowIds];
    } 
    else{
        return ['product_id' => $data['product_id'], 'ean' => $data['ean'], 'updated_rows' => 0, 'updated_rows_ids' => false];
    }  
}
