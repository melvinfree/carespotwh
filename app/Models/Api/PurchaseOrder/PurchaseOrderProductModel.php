...
class PurchaseOrderProductModel extends Model
{
    ...
    public function reverseInvoiceProductStock($data){
        ...
        if($count_already_reversed_product < $reversalProduct['quantity']){
            ...
            foreach ($ProductsToBeReversed as $stock_line){
                ...
                $this->db->table($this->stockTable)
                     ...
                     // Log Reverse Action (need to fix this)
                     $purchaseOrderTrack->logReverseAction($stock_line['id'],'reversed',$stock_line['order_id'] ?? null,$stock_line['invoice_out_id'] ?? null);
        ...
        elseif($count_already_reversed_product >= $reversalProduct['quantity']){
        

        $limit = $count_already_reversed_product - $reversalProduct['quantity'];
       ...
       $ProductsToBeReversed = $stockModel->where('invoice_in_id', $initial_invoice_id['id'])
            ...
            foreach ($ProductsToBeReversed as $stock_line){
                ...
                $this->db->table($this->stockTable)
                     ...
                     // Log Reverse Action
                     $purchaseOrderTrack->logReverseAction($stock_line['id'],'not_reversed',$stock_line['order_id'] ?? null,$stock_line['invoice_out_id'] ?? null);
